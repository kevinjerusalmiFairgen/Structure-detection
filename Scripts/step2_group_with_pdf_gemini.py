...
def build_prompt(use_pdf: bool) -> str:
    ...
    return (
        "INPUTS\n"
        "- SPSS metadata (COMPACT JSON) + ALLOWED_CODES (exact allowlist)\n"
        "- PDF questionnaire (for optional confirmation/refinement)\n\n"
        "OBJECTIVE\n"
        ...
        "- Do NOT output plain standalone variables.\n\n"
        "PROCESS (two phases)\n"
        "1) PHASE A — METADATA-FIRST (no PDF):\n"
        "   - Using ONLY metadata and ALLOWED_CODES, propose groups and recodes.\n"
        "   - Group criteria: identical/highly similar possible_answers, patterned codes (Q1_1..Q1_10), shared stems in metadata question_text, shared types.\n"
        "   - Recodes: include only true computed variables; set recode_from to exact codes in ALLOWED_CODES.\n"
        "   - Do NOT invent or normalize any code/text; do NOT include range (min/max) variables inside groups.\n"
        "2) PHASE B — VALIDATE/REFINE WITH PDF:\n"
        "   - Use the questionnaire to confirm stems/layout and adjust groups (split/merge) only if PDF provides strong evidence.\n"
        "   - Do NOT introduce any codes not in ALLOWED_CODES; do NOT alter codes/labels.\n"
        "   - Prefer metadata when PDF is ambiguous; PDF only refines names and completeness of groups.\n\n"
        "HARD CONSTRAINTS\n"
        "- SOURCE OF TRUTH: metadata. Never invent or normalize.\n"
        "- question_code values: must be EXACT matches from ALLOWED_CODES (letter-for-letter).\n"
        "- Preserve codes and possible_answers exactly as in metadata. Do not add, rename, reformat, reorder, slugify, or alter case/hyphens/underscores/digits.\n"
        "- Use metadata question_text verbatim for sub-questions.\n"
        "  • For group headers: use a stem/base suffixed with `_GROUP` (or `_GRID`). Never use a real variable code as a header.\n"
        "- Do not include range/min/max variables in groups.\n"
        "- Never treat routing/skip/display logic as recodes.\n\n"
        "DERIVED VARIABLES (RECODES)\n"
        "- Recodes can have a different structure than their sources (different question_type or possible_answers).\n"
        "- Include only true computed variables (indices/totals/bands).\n"
        "- recode_from: list exact variable codes from ALLOWED_CODES (not group headers).\n\n"
        "OUTPUT SCHEMA (JSON array only)\n"
        "[\n"
        ...
        "- ✅ No ranges/min/max inside groups.\n"
        "- ✅ recode_from sources are real codes from ALLOWED_CODES (not group headers).\n"
        "- ✅ PDF used only to confirm/refine; metadata remains the source of truth.\n"
    )
